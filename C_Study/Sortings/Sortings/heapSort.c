...
int* siftdown(int* array, int parentNode, int size) {
	...
	if (left <= size - 1 && array[left] > array[parentNode]) {	//left child node 가 트리 전체 크기(인덱스) 보다 작고 left child node의 값이 parent node의 값보다 크다면 수행
		largest = left;	//left child node의 인덱스를 가장 큰 값으로 저장
	}//end if

	if (right <= size - 1 && array[right] > array[largest]) {	//right child node 가 트리 전체 크기(인덱스) 보다 작고 right child node의 값이 parent node의 값보다 크다면 수행
		largest = right;	//right child node의 인덱스를 가장 큰 값으로 저장
	}//end if

	if (largest != parentNode) {	//가장 큰 값의 인덱스가 parent node의 인덱스와 다르면 수행
...
int* heapify(int* array, int size) {	//max-heap tree (parent node가 child node보다 큼)
