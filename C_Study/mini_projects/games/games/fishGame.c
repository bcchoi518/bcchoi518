...
int main(void) {
	...
	cursor = arrayFish; // cursor[0] ... cursor[5] 현재 선택된 어항
	...
	while (1) {
		...
		// 사용자가 입력한 어항에 물을 준다
		// 1. 어항의 물이 0 이면? 물을 주지 않는다... 이미 물고기가.. ㅠㅠ 흑흑..
		if (cursor[num - 1] <= 0) {
		...
		if (totalElapsedTime / 20 > level - 1) {
			...
			printf(" *** 축 렙레업 ! 기존 %d 레벨에서 %d 레벨로 업그레이드 ***\n\n", level - 1, level);
...
void decreaseWater(long elapsedTime) {
	for (int i = 0; i < 6; i++) {
		arrayFish[i] -= (level * 2 * (int)elapsedTime);	// 2 : 난이도 조절을 위한 값
		if (arrayFish[i] < 0) {
			arrayFish[i] = 0;
		}
	}
